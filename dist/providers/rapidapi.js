...
export class RapidApiProvider {
    ...
    // ---------------------------------------------------------------- transport
    async request(path, params, cacheTtlMs = 60_000) {
        ...
        try {
            ...
            if (response.status === 401 || response.status === 403) {
                throw new RailDataError('UPSTREAM_AUTH', 'The rail data provider rejected the API key.', 'Check IRCTC_RAPIDAPI_KEY and that your RapidAPI subscription covers this host.');
            }
            if (response.status === 429) {
                throw new RailDataError('RATE_LIMITED', 'The rail data provider is rate limiting this key.', 'Wait and retry, or upgrade the RapidAPI plan.');
            }
            if (!response.ok) {
                throw new RailDataError('UPSTREAM_ERROR', `Rail data provider returned HTTP ${response.status}.`);
            }
            const body = (await response.json());
            if (body && body.status === false) {
                throw new RailDataError('UPSTREAM_NO_DATA', typeof body.message === 'string' && body.message
                    ? body.message
            ...
            if (error instanceof Error && error.name === 'AbortError') {
                throw new RailDataError('UPSTREAM_TIMEOUT', `Rail data provider did not respond within ${this.timeoutMs}ms.`);
            }
            throw new RailDataError('UPSTREAM_UNREACHABLE', `Could not reach the rail data provider: ${error.message}`);
        }
        finally {
            clearTimeout(timer);
        }
    ...
    async withFallback(live, offline) {
        try {
            return await live();
        }
        catch (error) {
    ...
    async searchStations(query, limit) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v1/searchStation', { query }, 24 * 60 * 60 * 1000);
    ...
    async findTrainsBetweenStations(query) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v3/trainBetweenStations', {
            ...
            return query.travelClass
                ? results.filter((r) => r.train.classes.includes(query.travelClass))
                : results;
        }, () => this.fallback.findTrainsBetweenStations(query));
    }
    async getTrainSchedule(trainNumber) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v1/getTrainSchedule', { trainNo: trainNumber.trim() }, 60 * 60 * 1000);
            return normaliseSchedule(trainNumber, data);
        }, () => this.fallback.getTrainSchedule(trainNumber));
    }
    async getAvailability(query) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v1/checkSeatAvailability', {
    ...
    async getFare(query) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v2/getFare', {
                trainNo: query.trainNumber.trim(),
                fromStationCode: query.fromStationCode.toUpperCase(),
                toStationCode: query.toStationCode.toUpperCase(),
            }, 60 * 60 * 1000);
    ...
    async getPnrStatus(pnr) {
        ...
        return this.withFallback(async () => {
            const data = await this.request('/api/v3/getPNRStatus', { pnrNumber: clean }, 60 * 1000);
            return normalisePnr(clean, data);
        }, () => this.fallback.getPnrStatus(clean));
    }
    async getLiveTrainStatus(trainNumber, startDate) {
        return this.withFallback(async () => {
            const data = await this.request('/api/v1/liveTrainStatus', { trainNo: trainNumber.trim(), startDay: startDate }, 60 * 1000);
    ...
    for (const item of items) {
        const code = typeof item === 'string'
            ...
            : typeof item === 'object' && item !== null
                ? str(item, 'class', 'classType', 'code').toUpperCase()
                : '';
        if (valid.has(code) && !out.includes(code)) {
            out.push(code);
        }
    }
    return out;
}
function parseRunsOn(row) {
    ...
    const flags = keys.map((key) => {
        ...
        if (typeof value === 'boolean')
            return value;
        if (typeof value === 'string')
            return value.trim().toUpperCase() === 'Y' || value === '1';
        if (typeof value === 'number')
            return value === 1;
        return false;
    });
...
function normaliseTrainBetween(row, query) {
    ...
    const train = {
        ...
        runsOn,
        classes: classes.length > 0 ? classes : ['SL', '3A', '2A'],
    };
    return {
        train,
...
function normaliseSchedule(trainNumber, data) {
    const rows = (data['route'] ?? data['stations'] ?? data['data'] ?? []);
    if (!Array.isArray(rows) || rows.length === 0) {
        throw new RailDataError('NO_SCHEDULE', `No schedule returned for train ${trainNumber}.`);
    }
    const stops = rows.map((row, index) => {
        const arrival = str(row, 'arrivalTime', 'arrival', 'sta');
        const departure = str(row, 'departureTime', 'departure', 'std');
        const clean = (t) => !t || t === '--' || t === '00:00:00' || t.toLowerCase() === 'source' || t.toLowerCase() === 'destination'
            ? null
            : t.slice(0, 5);
        return {
...
function classifyAvailability(text) {
    ...
    if (upper.includes('WL')) {
        ...
        const result = { status: 'WAITLIST' };
        if (match?.[1])
            result.count = Number(match[1]);
        return result;
    }
    if (upper.includes('CHART'))
        return { status: 'CHART_PREPARED' };
    return { status: 'NOT_APPLICABLE' };
}
function normaliseAvailability(query, data) {
    const rows = Array.isArray(data) ? data : (data['data'] ?? []);
    ...
    if (!statusText) {
        throw new RailDataError('NO_AVAILABILITY', `No availability returned for train ${query.trainNumber} in ${query.travelClass} on ${query.date}.`);
    }
    ...
    if (chance > 0)
        result.confirmationChance = Math.min(1, chance / 100);
    return result;
}
function normaliseFare(query, data) {
    const general = (data['general'] ?? data['fare'] ?? data['data']);
    const rows = Array.isArray(general) ? general : [];
    const match = rows.find((row) => str(row, 'classType', 'class_type', 'class').toUpperCase() === query.travelClass);
    if (!match)
        return null;
    ...
    const baseFare = num(match, 'baseFare') ||
        Math.max(0, total - reservationCharge - superfastCharge - gst - cateringCharge - dynamicSurcharge);
    return {
        trainNumber: query.trainNumber,
        travelClass: query.travelClass,
        quota: query.quota,
...
function normalisePnr(pnr, data) {
    const passengerRows = (data['passengerList'] ?? data['passengers'] ?? []);
    const passengers = (Array.isArray(passengerRows) ? passengerRows : []).map((row, index) => {
        const current = str(row, 'currentStatus', 'current_status') ||
            str(row, 'currentStatusDetails', 'current_status_details') ||
            'UNKNOWN';
        const entry = {
            serial: num(row, 'passengerSerialNumber') || index + 1,
            bookingStatus: str(row, 'bookingStatus', 'booking_status', 'bookingStatusDetails') || current,
        ...
        if (berth)
            entry.berth = berth;
        if (berthType)
            entry.berthType = berthType;
        return entry;
    });
    if (passengers.length === 0) {
        throw new RailDataError('PNR_NOT_FOUND', `No booking found for PNR ${pnr}.`, 'PNRs are purged a few days after the journey date.');
    }
    ...
    const travelClass = (str(data, 'journeyClass', 'class') || 'SL').toUpperCase();
    const quota = (str(data, 'quota', 'quotaCode') || 'GN').toUpperCase();
    const result = {
        pnr,
    ...
    if (bookingFare)
        result.bookingFare = bookingFare;
    if (ticketFare)
        result.ticketFare = ticketFare;
    return result;
}
function normaliseLiveStatus(trainNumber, startDate, data) {
    const rows = (data['upcoming_stations'] ?? data['route'] ?? data['stations'] ?? []);
    const toUpdate = (row, departed) => {
        const clean = (t) => (!t || t === '--' ? null : t.slice(0, 5));
        return {
    ...
    const passedRows = (data['passed_stations'] ?? []);
    ...
    const currentStationName = str(data, 'current_station_name', 'currentStationName') || passed[passed.length - 1]?.stationName || null;
    const currentStationCode = str(data, 'current_station_code', 'currentStationCode').toUpperCase() ||
