 ...
 */
export class ConfirmTktProvider {
    name = 'confirmtkt';
    isLive = true;
    http;
    ...
    // ----------------------------------------------------------------- search
    async search(fromCode, toCode, isoDate) {
        const url = `${API_HOST}/api/v1/trains/search` +
        ...
        const json = await this.http.getJson(url, { headers: API_HEADERS, cacheTtlMs: 60_000, label: 'ConfirmTkt' });
        if (json.error) {
            throw new RailDataError('UPSTREAM_NO_DATA', `ConfirmTkt: ${json.error.message ?? json.error.code ?? 'request rejected'}`);
        }
        return json.data?.trainList ?? [];
    }
    async searchStations(query, limit) {
        const url = `${API_HOST}/api/v2/trains/stations/auto-suggestion` +
    ...
    toTrainBetween(t, query, fromCode, toCode) {
        if (!t.trainNumber)
            return null;
        ...
        const train = {
            ...
            runsOn,
            classes: classes.length > 0 ? classes : ['SL'],
    ...
    async getTrainSchedule(trainNumber) {
        throw new RailDataError('NOT_SUPPORTED', `ConfirmTkt does not expose a stop-by-stop schedule for train ${trainNumber}.`);
    }
    // ----------------------------------------------- availability and fares
    /** Locate one class in the availability cache for a train on a route/date. */
    async findClassEntry(trainNumber, fromCode, toCode, isoDate, travelClass) {
        ...
        if (!train) {
            throw new RailDataError('TRAIN_NOT_ON_ROUTE', `Train ${trainNumber} does not run between ${fromCode.toUpperCase()} and ${toCode.toUpperCase()} on ${isoDate}.`, list.length > 0
        ...
        if (!entry) {
            ...
            throw new RailDataError('CLASS_NOT_AVAILABLE', `Train ${trainNumber} has no ${travelClass} availability on this route.`, offered ? `Classes offered: ${offered}.` : undefined);
        }
        return { entry, train };
    }
    async getAvailability(query) {
        const { entry } = await this.findClassEntry(query.trainNumber, query.fromStationCode, query.toStationCode, query.date, query.travelClass);
        const statusText = entry.availabilityDisplayName?.trim() || entry.availability?.trim() || 'UNKNOWN';
        const { status, count } = classifyAvailability(`${entry.availability ?? ''} ${statusText}`);
        const result = {
            trainNumber: query.trainNumber,
            travelClass: query.travelClass,
            quota: entry.quota?.toUpperCase() || query.quota,
            date: query.date,
            status,
            statusText,
        };
    ...
    async getFare(query) {
        if (!query.date) {
            throw new RailDataError('DATE_REQUIRED', 'ConfirmTkt quotes fares per journey date; no date was supplied.');
        }
        const { entry } = await this.findClassEntry(query.trainNumber, query.fromStationCode, query.toStationCode, query.date, query.travelClass);
        const total = Number(entry.fare);
        if (!Number.isFinite(total) || total <= 0) {
            throw new RailDataError('NO_FARE', `ConfirmTkt did not quote a ${query.travelClass} fare for train ${query.trainNumber}.`);
        }
    ...
    async getPnrStatus(pnr) {
        ...
        if (!match?.[1]) {
            throw new RailDataError('PNR_PARSE_FAILED', `Could not read a PNR result for ${clean}.`, 'The upstream page layout may have changed, or the PNR may not exist. PNRs are purged a few days after travel.');
        }
        let data;
        try {
    ...
    async getLiveTrainStatus(trainNumber) {
        throw new RailDataError('NOT_SUPPORTED', `ConfirmTkt does not expose a machine-readable running status for train ${trainNumber}.`);
    }
}
// ------------------------------------------------------------- helpers
...
function normalisePnrPayload(pnr, data) {
    const rawList = data['passengerList'] ??
        data['passengers'] ??
        data['PassengerStatus'] ??
        [];
    const rows = Array.isArray(rawList) ? rawList : [];
    const passengers = rows.map((row, i) => {
        const current = str(row, 'currentStatus', 'CurrentStatus', 'current_status') || 'UNKNOWN';
        const entry = {
    ...
    if (passengers.length === 0) {
        throw new RailDataError('PNR_NOT_FOUND', `No booking found for PNR ${pnr}.`, 'Check the number. PNRs are purged a few days after the journey date.');
    }
    ...
    return {
        ...
        travelClass: (str(data, 'journeyClass', 'Class', 'className') || 'SL').toUpperCase(),
        quota: (str(data, 'quota', 'Quota') || 'GN').toUpperCase(),
        chartPrepared: chartRaw === true ||
            String(chartRaw).toUpperCase().includes('PREPARED') ||
            String(chartRaw).toUpperCase() === 'TRUE',
        passengers,
    };
}
//# sourceMappingURL=confirmtkt.js.map