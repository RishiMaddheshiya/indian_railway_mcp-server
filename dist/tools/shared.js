 ...
 */
export async function resolveStation(provider, input, label) {
    ...
    if (matches.length === 0) {
        ...
        if (lookupError instanceof RailDataError)
            throw lookupError;
        throw new RailDataError('STATION_NOT_FOUND', `Could not resolve ${label} station "${input}".`, 'Use search_stations to find the right code.');
    }
    ...
    const exactish = matches.find((m) => m.code === upper || m.name.toLowerCase() === value.toLowerCase());
    if (exactish)
        return exactish;
    if (matches.length > 1) {
        const second = matches[1];
        if (second && second.name.toLowerCase().startsWith(value.toLowerCase())) {
            throw new RailDataError('AMBIGUOUS_STATION', `"${input}" matches more than one station.`, matches.map((m) => `${m.code} = ${m.name}`).join('; '));
        }
    }
    return first;
}
