...
const GST_CLASSES = new Set(['1A', '2A', '3A', '3E', 'EC', 'CC']);
...
export function computeFare(input) {
    const { travelClass, quota, distanceKm, trainType, ageCategory = 'adult', } = input;
    ...
    const dynamicSurcharge = premium && quota !== 'HP' ? round5(baseFare * 0.1) : 0;
    const taxable = baseFare + reservationCharge + superfastCharge + cateringCharge + dynamicSurcharge;
