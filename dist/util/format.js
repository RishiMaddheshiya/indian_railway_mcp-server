...
export const rupees = (amount) => `Rs ${amount.toLocaleString('en-IN')}`;
export function className(code) {
    return TRAVEL_CLASSES[code] ?? code;
}
export function quotaName(code) {
    return QUOTAS[code] ?? code;
}
export function runDays(runsOn) {
    if (runsOn.every(Boolean))
        return 'Daily';
...
export function formatStations(stations) {
    ...
    return table(['Code', 'Station', 'State', 'Zone'], stations.map((s) => [s.code, s.name, s.state ?? '-', s.zone ?? '-']));
}
export function formatTrainsBetween(results, date) {
    if (results.length === 0)
        return 'No direct trains found for this pair.';
    ...
    const header = table(['Train', 'Name', 'Dep', 'Arr', 'Duration', 'Classes', `Runs ${date}`], rows);
    const notRunning = results.filter((r) => !r.runsOnDate);
    const footer = notRunning.length > 0
...
export function formatSchedule(train, stops) {
    const head = [
        ...
        .filter(Boolean)
        .join('\n');
    const body = table(['#', 'Station', 'Arr', 'Dep', 'Halt', 'Km', 'Day', 'PF'], stops.map((s, i) => [
        String(i + 1),
...
export function formatAvailability(a) {
    const chance = a.confirmationChance === undefined
        ? ''
...
export function formatFare(f) {
    ...
    const hasBreakdown = f.baseFare > 0 || f.reservationCharge > 0 || f.superfastCharge > 0 || f.gst > 0;
    if (!hasBreakdown) {
        return [
            header,
            '',
    ...
    if (f.superfastCharge > 0)
        rows.push(['Superfast charge', rupees(f.superfastCharge)]);
    if (f.cateringCharge > 0)
        rows.push(['Catering', rupees(f.cateringCharge)]);
    if (f.dynamicSurcharge > 0)
        rows.push(['Dynamic/flexi surcharge', rupees(f.dynamicSurcharge)]);
    if (f.gst > 0)
        rows.push(['GST (5%)', rupees(f.gst)]);
    rows.push(['**Total per passenger**', `**${rupees(f.totalFare)}**`]);
    return [header, '', table(['Component', 'Amount'], rows)].join('\n');
}
export function formatPnr(p) {
    const head = [
    ...
    const body = table(['Pax', 'Booking status', 'Current status'], p.passengers.map((pax) => [
        String(pax.serial),
        pax.bookingStatus,
        pax.currentStatus,
    ]));
...
export function formatLiveStatus(s, upcomingLimit = 8) {
    ...
    const body = table(['Station', 'Sch. arr', 'Exp. arr', 'Delay', 'PF'], s.upcomingStations.slice(0, upcomingLimit).map((u) => [
    ...
    const more = s.upcomingStations.length > upcomingLimit
