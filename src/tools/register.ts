...
const travelClassEnum = z.enum(
  Object.keys(TRAVEL_CLASSES) as [TravelClass, ...TravelClass[]],
);
const quotaEnum = z.enum(Object.keys(QUOTAS) as [Quota, ...Quota[]]);

const stationField = (role: string) =>
  z
    .string()
    .min(2)
    .describe(
      `${role} station: either the station code (e.g. "NDLS") or the name (e.g. "New Delhi").`,
    );
...
const dateField = z
  ...
  .optional()
  .describe(
    'Journey date as YYYY-MM-DD, or "today" / "tomorrow". Defaults to today in IST.',
  );
 ...
 */
function provenance(provider: RailProvider, kind: 'static' | 'live'): string {
  if (provider.isLive) {
...
export function registerTools(server: McpServer, provider: RailProvider): void {
  // ------------------------------------------------------------- stations
  ...
  server.registerTool(
    ...
    async ({ query, limit }) =>
      guard(async () => {
        ...
        return ok(
          `Stations matching "${query}":\n\n${formatStations(stations)}${provenance(provider, 'static')}`,
  ...
  server.registerTool(
    ...
    async ({ from, to, date, travelClass, sortBy, onlyRunningOnDate }) =>
      guard(async () => {
        ...
        const query: Parameters<RailProvider['findTrainsBetweenStations']>[0] = {
        ...
        if (onlyRunningOnDate !== false) {
          ...
          if (running.length > 0) results = running;
        }
        ...
        if (sortBy === 'duration') {
          ...
          results.sort(
            (a, b) =>
              a.arrivalDayOffset * 1440 - b.arrivalDayOffset * 1440 ||
  ...
  server.registerTool(
    ...
    async ({ trainNumber, from, to, date, travelClass, quota }) =>
      guard(async () => {
        const journeyDate = parseDate(date);
        ...
        return ok(
          `${formatAvailability(availability)}\n\nRoute: ${origin.name} (${origin.code}) -> ${destination.name} (${destination.code})${provenance(provider, 'live')}`,
          { availability, from: origin, to: destination },
        );
      }),
  );
  ...
  server.registerTool(
    ...
    async ({ trainNumber, from, to, date, travelClass, quota, ageCategory, passengers }) =>
      guard(async () => {
        const journeyDate = parseDate(date);
        const origin = await resolveStation(provider, from, 'boarding');
        const destination = await resolveStation(provider, to, 'alighting');

        const query: Parameters<RailProvider['getFare']>[0] = {
          trainNumber: trainNumber.trim(),
          fromStationCode: origin.code,
          toStationCode: destination.code,
          travelClass,
  ...
  server.registerTool(
    ...
    async ({ pnr }) =>
      guard(async () => {
        ...
        const confirmed = status.passengers.filter((p) =>
          p.currentStatus.toUpperCase().startsWith('CNF'),
        ).length;
        const summary = `${confirmed} of ${status.passengers.length} passenger(s) confirmed.`;
        return ok(
          `${formatPnr(status)}\n\n${summary}${provenance(provider, 'live')}`,
          { pnr: status, confirmedCount: confirmed },
        );
      }),
  );
  ...
  server.registerTool(
    ...
    async ({ from, to, date, preferredClasses, quota, passengers, maxOptions, optimiseFor }) =>
      guard(async () => {
        const journeyDate = parseDate(date);
        const origin = await resolveStation(provider, from, 'origin');
        const destination = await resolveStation(provider, to, 'destination');
        const classes = preferredClasses ?? (['3A', 'SL'] as TravelClass[]);
        ...
        if (candidates.length === 0) {
          return ok(
            `No direct trains found from ${origin.name} (${origin.code}) to ${destination.name} (${destination.code}) on ${journeyDate}. Consider a nearby junction or a connecting journey.${provenance(provider, 'static')}`,
            { date: journeyDate, from: origin, to: destination, options: [] },
          );
        }
        ...
        const score = (opt: (typeof options)[number]): number => {
        ...
        const body =
          rows.length > 0
            ? table(
                ['Train', 'Name', 'Timing', 'Duration', 'Class', 'Availability', `Fare (${paxCount} pax)`],
                rows,
              )
      ...
      guard(async () => {
        ...
        if (which === 'classes' || which === 'both') {
          parts.push(
            `**Travel classes**\n\n${table(
              ['Code', 'Meaning'],
              Object.entries(TRAVEL_CLASSES).map(([c, n]) => [c, n]),
            )}`,
          );
        }
        if (which === 'quotas' || which === 'both') {
          parts.push(
            `**Quotas**\n\n${table(
              ['Code', 'Meaning'],
              Object.entries(QUOTAS).map(([c, n]) => [c, n]),
            )}`,
          );
        }
...
function scoreAvailability(a: Availability | null): number {
  if (!a) return 0;
  switch (a.status) {
    case 'AVAILABLE':
      return 3;
