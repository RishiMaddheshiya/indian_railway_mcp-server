import type { ScheduleStop, Train, TravelClass } from '../types.js';
...
/** Compact stop tuple: [code, name, arrival, departure, distanceKm, day, platform?]. */
type RawStop = [string, string, string | null, string | null, number, number, string?];

const ALL_DAYS = [true, true, true, true, true, true, true];
const days = (...idx: number[]): boolean[] =>
  ALL_DAYS.map((_, i) => idx.includes(i));

function toStops(raw: RawStop[]): ScheduleStop[] {
...
function haltBetween(arrival: string | null, departure: string | null): number {
...
function makeTrain(
  ...
  if (!first || !last) throw new Error(`Train ${number} has no stops`);
  return {
    train: {
      number,
      name,
...
export const TRAIN_BY_NUMBER = new Map<string, TrainRecord>(
  TRAINS.map((t) => [t.train.number, t]),
);
