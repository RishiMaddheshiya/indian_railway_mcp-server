 ...
 * IRCTC partner API.
 */
export class ErailProvider implements RailProvider {
  readonly name = 'erail';
  ...
  private async fetchTrains(params: string, label: string): Promise<string> {
  ...
  async searchStations(query: string): Promise<Station[]> {
    throw new RailDataError(
      'NOT_SUPPORTED',
      `eRail has no station search endpoint (query "${query}").`,
    );
  }
  ...
  async findTrainsBetweenStations(query: TrainsBetweenQuery): Promise<TrainBetweenStations[]> {
    ...
    const body = await this.fetchTrains(
      `Station_From=${encodeURIComponent(from)}&Station_To=${encodeURIComponent(to)}`,
      'eRail trains-between',
    );
    if (!body) return [];
  ...
  async getTrainSchedule(trainNumber: string): Promise<{ train: Train; stops: ScheduleStop[] }> {
    ...
    const info = await this.fetchTrains(
      `TrainNo=${encodeURIComponent(number)}`,
      'eRail train lookup',
    );
    ...
    if (!trainId) {
      throw new RailDataError(
        'NO_SCHEDULE',
        `Could not resolve an eRail route id for train ${number}.`,
      );
    }

    // Step 2: fetch the route for that id.
    const routeBody = await this.http.getText(
      `${HOST}/data.aspx?Action=TRAINROUTE&Password=2012&Data1=${encodeURIComponent(trainId)}&Data2=0&Cache=true`,
      { cacheTtlMs: 24 * 60 * 60 * 1000, label: 'eRail route' },
    );
  ...
  async getAvailability(query: AvailabilityQuery): Promise<Availability> {
    throw new RailDataError(
      'NOT_SUPPORTED',
      `eRail does not expose live seat availability for train ${query.trainNumber}.`,
    );
  }
  ...
  async getFare(query: FareQuery): Promise<FareBreakdown> {
    throw new RailDataError(
      'NOT_SUPPORTED',
      `eRail does not expose a per-class fare lookup for train ${query.trainNumber}.`,
    );
  }
  ...
  async getLiveTrainStatus(trainNumber: string): Promise<LiveTrainStatus> {
    throw new RailDataError(
      'NOT_SUPPORTED',
      `eRail does not expose running status for train ${trainNumber}.`,
    );
  }
}
...
function hhDotMmToMinutes(raw: string | undefined): number {
  if (!raw) return 0;
 ...
 */
function parseErailRunDays(mask: string | undefined): boolean[] {
  const fallback = [true, true, true, true, true, true, true];
  if (!mask || mask.trim().length < 7) return fallback;
  const bits = mask.trim();

  // maskIndexForJsDay[jsDay] where jsDay 0 = Sunday.
  const maskIndexForJsDay = (jsDay: number): number => (jsDay <= 2 ? jsDay + 4 : jsDay - 3);
 ...
 */
function parseClassesFromAvailabilityBlock(chunk: string): TravelClass[] {
  const out: TravelClass[] = [];
  for (const m of chunk.matchAll(/(^|[~|])([0-9A-Z]{2})\s*:/g)) {
    const code = (m[2] ?? '').toUpperCase();
    if (VALID_CLASSES.has(code) && !out.includes(code as TravelClass)) {
