...
export class MockProvider implements RailProvider {
  ...
  async findTrainsBetweenStations(query: TrainsBetweenQuery): Promise<TrainBetweenStations[]> {
    ...
    for (const record of TRAINS) {
      ...
      if (!origin?.departure || !dest?.arrival) continue;

      if (query.travelClass && !record.train.classes.includes(query.travelClass)) continue;

      const dayOffset = dest.day - origin.day;
      const durationMinutes =
        toMinutes(dest.arrival) - toMinutes(origin.departure) + dayOffset * 1440;
  ...
  async getTrainSchedule(trainNumber: string): Promise<{ train: Train; stops: ScheduleStop[] }> {
    ...
    if (!record) {
      throw new RailDataError(
        'TRAIN_NOT_FOUND',
        `Train ${trainNumber} is not in the bundled sample timetable.`,
        'The offline dataset covers a sample of popular trains. Configure a live provider (IRCTC_PROVIDER=rapidapi) for full coverage.',
      );
    }
  ...
  async getAvailability(query: AvailabilityQuery): Promise<Availability> {
    ...
    if (!record) {
      throw new RailDataError(
        'TRAIN_NOT_FOUND',
        `Train ${query.trainNumber} is not in the bundled sample timetable.`,
      );
    }
    if (!record.train.classes.includes(query.travelClass)) {
      throw new RailDataError(
        'CLASS_NOT_AVAILABLE',
        `Train ${record.train.number} (${record.train.name}) does not have class ${query.travelClass}.`,
        `Available classes: ${record.train.classes.join(', ')}.`,
      );
    }
    ...
    if (daysAhead === 0) {
      ...
      if (status === 'CHART_PREPARED') statusText = `${statusText} (CHART PREPARED)`;
    }
  ...
  async getFare(query: FareQuery): Promise<FareBreakdown> {
    ...
    if (!fromStop || !toStop) {
      throw new RailDataError(
        ...
        `Train ${record.train.number} does not stop at ${!fromStop ? from : to}.`,
        `Route: ${record.stops.map((s) => s.stationCode).join(' -> ')}`,
      );
    }
    if (fromStop.distanceKm >= toStop.distanceKm) {
      throw new RailDataError(
        'WRONG_DIRECTION',
        `Train ${record.train.number} runs ${record.train.fromStationCode} -> ${record.train.toStationCode}; ${from} comes after ${to} on that route.`,
      );
    }
    if (!record.train.classes.includes(query.travelClass)) {
      throw new RailDataError(
        'CLASS_NOT_AVAILABLE',
        `Train ${record.train.number} does not have class ${query.travelClass}.`,
        `Available classes: ${record.train.classes.join(', ')}.`,
      );
    }
  ...
  async getPnrStatus(pnr: string): Promise<PnrStatus> {
    ...
    if (clean.length !== 10) {
      throw new RailDataError(
        'INVALID_PNR',
        'A PNR must be exactly 10 digits.',
        `Received "${pnr}" (${clean.length} digits).`,
      );
    }

    const rng = seededRandom(`pnr:${clean}`);
    const record = TRAINS[randomInt(rng, 0, TRAINS.length - 1)];
    if (!record) throw new RailDataError('NO_DATA', 'No sample trains available.');
    ...
    const travelClass = record.train.classes[randomInt(rng, 0, record.train.classes.length - 1)] as TravelClass;
  ...
  async getLiveTrainStatus(trainNumber: string, startDate: string): Promise<LiveTrainStatus> {
    ...
    const passed: LiveStationUpdate[] = [];
    const upcoming: LiveStationUpdate[] = [];
    ...
    record.stops.forEach((stop, index) => {
      ...
      const stopDelay = Math.max(
        0,
        Math.round(baseDelay * (0.4 + progress * 0.9) + randomInt(rng, -6, 8)),
      );
      ...
      const update: LiveStationUpdate = {
